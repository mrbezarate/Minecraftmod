...
const CONFIG = {
  ...
  USERNAME: process.env.MC_USERNAME || 'BotBibob5',
...
async function handleMine(block, toolType) {
  ...
  log(`⛏️  Добываю ${blockName} на позиции ${block.position}`)
  ...
  try {
    ...
    if (!toolType) {
      ...
      if (name.includes('log') || name.includes('wood') || name.includes('planks') || name.includes('leaves')) {
        toolType = 'axe'
        log(`🔧 Автоматически выбран инструмент: топор (для дерева)`)
        ...
        toolType = 'pickaxe'
        log(`🔧 Автоматически выбран инструмент: кирка (для камня/руды)`)
      } else if (name.includes('dirt') || name.includes('sand') || name.includes('gravel') || name.includes('clay')) {
        toolType = 'shovel'
        log(`🔧 Автоматически выбран инструмент: лопата (для земли/песка)`)
      } else {
        log(`⚠️  Неизвестный тип блока, добываю без специального инструмента`)
      }
    }
    
    // Подбираем лучший инструмент
    if (toolType) {
      log(`🔧 Подбираю лучший инструмент: ${toolType}`)
      const equipped = await equipBestTool(toolType)
      if (!equipped) {
        log(`⚠️  Не найден инструмент ${toolType}, добываю руками`)
      }
    }
    
    log(`🚶 Иду к блоку ${blockName}...`)
    await goToPositionSafe(block.position, 1)
    log(`⛏️  Начинаю добычу ${blockName}...`)
    await digBlockReliable(block, toolType)
    
    // Сохраняем ресурс в память
    if (block.name) {
      Memory.saveResource(block.name, block.position)
    }
    
    log(`✅ Успешно добыл ${blockName}`)
    return 'done'
  } catch (e) {
...
async function handleCraft(itemName, quantity) {
  ...
  try {
    ...
    // Ищем рецепты
    let recipes = bot.recipesFor(itemDef.id, null, 1)
    
    // Если рецепты не найдены, пробуем через recipesAll
    if (!recipes || recipes.length === 0) {
      log(`⚠️  Рецепты через recipesFor не найдены, пробую recipesAll...`)
      recipes = bot.recipesAll(itemDef.id, null)
    }
    
    if (!recipes || recipes.length === 0) {
      log(`❌ Рецепт для ${itemName} не найден`)
      log(`💡 Возможно, этот предмет нельзя скрафтить или нужны специальные условия`)
      return 'failed'
    }
    
    const rec = recipes[0]
    log(`📋 Найден рецепт для ${itemName}, требуется верстак: ${rec.requiresTable || false}`)
    
    // Проверяем ингредиенты
    log(`📋 Проверяю ингредиенты для ${itemName}...`)
    const missingIngredients = []
    
    for (const ing of rec.ingredients) {
      const ingName = mcData.items[ing.id]?.name
      if (!ingName) {
        log(`⚠️  Неизвестный ингредиент с ID: ${ing.id}`)
        continue
      }
      
      const have = countItem(ingName)
      const need = ing.count * quantity
      
      log(`   ${ingName}: есть ${have}, нужно ${need}`)
      
      if (have < need) {
        missingIngredients.push({ name: ingName, have, need })
        log(`⚠️  Не хватает ${ingName}: есть ${have}, нужно ${need}`)
      }
    }
    
    if (missingIngredients.length > 0) {
      log(`❌ Недостаточно ингредиентов для крафта ${itemName}`)
      // Для некоторых ингредиентов можно попробовать добыть
      for (const missing of missingIngredients) {
        if (missing.name.includes('log') || missing.name.includes('planks') || missing.name.includes('stone') || missing.name.includes('ore')) {
          log(`💡 Пытаюсь добыть ${missing.name}...`)
          TaskQueue.add({
            type: 'mine',
            blockType: missing.name,
            priority: TaskQueue.priority.RESOURCE
          })
        }
      }
    ...
    if (rec.requiresTable) {
      log(`🔨 Нужен верстак, проверяю наличие...`)
      const hasTable = await ensureCraftingTable()
      if (!hasTable) {
        log(`❌ Не удалось получить верстак для крафта`)
        return 'failed'
      }
    }
    
    const tableBlock = rec.requiresTable 
      ? bot.findBlock({ 
          matching: (b) => {
            const def = mcData.blocks[b.type]
            return def && (def.name === 'crafting_table' || def.name === 'minecraft:crafting_table')
          }, 
          maxDistance: 6 
        })
      : null
    
    if (rec.requiresTable && !tableBlock) {
      log(`❌ Верстак не найден рядом (нужно в радиусе 6 блоков)`)
      log(`💡 Попробуйте поставить верстак рядом с ботом`)
      return 'failed'
    }
    
    if (rec.requiresTable && tableBlock) {
      log(`✅ Верстак найден на позиции ${tableBlock.position}`)
    }
    
    log(`🔨 Начинаю крафт ${itemName} x${quantity}...`)
    try {
      await timeoutPromise(bot.craft(rec, quantity, tableBlock), 15000, 'craft timeout')
      log(`✅ Успешно скрафтил ${itemName} x${quantity}`)
      return 'done'
    } catch (craftError) {
      log(`❌ Ошибка при выполнении крафта: ${craftError.message}`)
      // Пробуем альтернативный рецепт если есть
      if (recipes.length > 1) {
        log(`💡 Пробую альтернативный рецепт...`)
        try {
          await timeoutPromise(bot.craft(recipes[1], quantity, tableBlock), 15000, 'craft timeout')
          log(`✅ Успешно скрафтил ${itemName} x${quantity} альтернативным способом`)
          return 'done'
        } catch (e2) {
          log(`❌ Альтернативный рецепт тоже не сработал: ${e2.message}`)
        }
      }
      return 'failed'
    }
...
async function digBlockReliable(block, toolType = null, timeoutMs = 20000) {
...
async function ensureCraftingTable() {
  if (countItem('crafting_table') > 0) return true
  
  const plankNames = Object.keys(mcData.itemsByName).filter(n => n.includes('_planks'))
  let plankCount = plankNames.reduce((s, n) => s + countItem(n), 0)
  
  if (plankCount < 4) {
    for (const pname of plankNames) {
      const pid = mcData.itemsByName[pname]?.id
      if (!pid) continue
      const recs = bot.recipesFor(pid, null, 1)
      if (recs && recs.length) {
        try {
          await timeoutPromise(bot.craft(recs[0], 1, null), 8000, 'craft planks timeout')
          break
        } catch (e) { }
      }
    }
  ...
  const recs = bot.recipesFor(tableId, null, 1)
  if (!recs || !recs.length) return false
  
  try {
    await timeoutPromise(bot.craft(recs[0], 1, null), 10000, 'craft table timeout')
    return true
  } catch (e) {
    return false
  }
